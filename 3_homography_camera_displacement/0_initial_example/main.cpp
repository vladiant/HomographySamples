...
Mat computeHomography(const Mat &R1, const Mat &tvec1, const Mat &R2,
                      const Mat &tvec2, const double d_inv, const Mat &normal) {
  Mat homography =
      R2 * R1.t() + d_inv * (-R2 * R1.t() * tvec1 + tvec2) * normal.t();
  return homography;
}
...
void homographyFromCameraDisplacement(const string &img1Path,
  ...
  Mat img_draw_poses;
  drawFrameAxes(img1_copy_pose, cameraMatrix, distCoeffs, rvec1, tvec1,
                2 * squareSize);
  drawFrameAxes(img2_copy_pose, cameraMatrix, distCoeffs, rvec2, tvec2,
                2 * squareSize);
  ...
  // Same but using absolute camera poses instead of camera displacement, just
  // for check
  Mat homography_euclidean2 =
      computeHomography(R1, tvec1, R2, tvec2, d_inv1, normal1);
  ...
  imshow("Warped image using homography computed from camera displacement",
         img1_warp_custom);
  ...
  imshow("Warped image using homography computed from absolute camera poses",
         img1_warp_custom2);
  waitKey();
}
const char *params =
...
int main(int argc, char *argv[]) {
  ...
  homographyFromCameraDisplacement(
      parser.get<String>("image1"), parser.get<String>("image2"), patternSize,
      squareSize, parser.get<String>("intrinsics"));
  return 0;
}
