...
void basicPanoramaStitching(const string &img1Path, const string &img2Path) {
  ...
  Mat c1Mo =
      (Mat_<double>(4, 4) << 0.9659258723258972, 0.2588190734386444, 0.0,
       1.5529145002365112, 0.08852133899927139, -0.3303661346435547,
       -0.9396926164627075, -0.10281121730804443, -0.24321036040782928,
       0.9076734185218811, -0.342020183801651, 6.130080699920654, 0, 0, 0, 1);
  //! [camera-pose-from-Blender-at-location-1]
  ...
  Mat cameraMatrix =
      (Mat_<double>(3, 3) << 700.0, 0.0, 320.0, 0.0, 700.0, 240.0, 0, 0, 1);
  //! [camera-intrinsics-from-Blender]
...
int main(int argc, char *argv[]) {
  ...
  basicPanoramaStitching(parser.get<String>("image1"),
