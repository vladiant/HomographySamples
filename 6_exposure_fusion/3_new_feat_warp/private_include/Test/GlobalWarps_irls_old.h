...
class GlobalWarps {
 ...
 private:
  ...
  void composeMatAndVectPersp(Vertex *before, Vertex *after, unsigned vertCnt,
                              F g, F h, F *mat, F *vect, unsigned weights[]);
...
bool GlobalWarps<Vertex, F>::regPersp(Vertex *before_points,
  ...
  *it++ = 0;  // h
  ...
  F old_mp_coeffs[TEST_GYRO_V_STAB_TRANSF_MAX_DIM];
  ...
  do {
    ...
    // std::cout << merr << "  " << std << "  " << N << std::endl;

    // ****************************** REMOVE OUTLIERS **********************
    for (int o = 0; o < vertCnt; o++) {
      if (err[o] > 3 * std && weights[o] == 1) {
        weights[o] = 0;
        --N;
      }
    }
    ...
    if (flag)
      flag = false;
    else if (oldmerr < merr) {
      for (int i = 0; i < TEST_GYRO_V_STAB_TRANSF_MAX_DIM; i++)
        mp_coeffs[i] = old_mp_coeffs[i];
      return true;
    }

    oldmerr = merr;
    for (int i = 0; i < TEST_GYRO_V_STAB_TRANSF_MAX_DIM; i++)
      old_mp_coeffs[i] = mp_coeffs[i];
