...
class GlobalWarps {
 public:
  ...
  bool regPerspRobust(Vertex *before_points, Vertex *after_points,
                      unsigned vertCnt, float sigma = 3.0);
 ...
 private:
  ...
  void composeMatAndVectPersp(Vertex *before, Vertex *after, unsigned vertCnt,
                              F g, F h, F *mat, F *vect, bool *weights);
...
/////////////////////////////////////////////////////////////////////////////////////////////////
template <typename Vertex, typename F>
bool GlobalWarps<Vertex, F>::regPerspRobust(Vertex *before_points,
                                            Vertex *after_points,
                                            unsigned vertCnt, float sigma) {
  F *it = mp_coeffs;
  *it++ = 1;  // a
  *it++ = 0;  // b
  *it++ = 0;  // c
  ...
  *it++ = 0;  // h
  ...
  // robust weights
  bool weights[vertCnt];
  for (int i = 0; i < vertCnt; i++) weights[i] = true;

  F mat[(TEST_GYRO_V_STAB_PERSP_DIM * (TEST_GYRO_V_STAB_PERSP_DIM + 1)) / 2];
  F vect[TEST_GYRO_V_STAB_PERSP_DIM];
  SolveReturnCode ret;

  int oldinlcnt = vertCnt;
  bool flag = false;
  int iter = 0;
  ...
  do {
    ...
    float resid[vertCnt];
    for (int i = 0; i < vertCnt; i++) resid[i] = 0.0;

    int inlcnt = 0;
    for (int o = 0; o < vertCnt; o++) {
      if (weights[o]) {
        float dx_tmp = (before_points[o].x() * mp_coeffs[0] +
                        before_points[o].y() * mp_coeffs[1] + mp_coeffs[2]);
        float dy_tmp = (before_points[o].x() * mp_coeffs[3] +
                        before_points[o].y() * mp_coeffs[4] + mp_coeffs[5]);
        float dz_tmp = (before_points[o].x() * mp_coeffs[6] +
                        before_points[o].y() * mp_coeffs[7] + 1);

        residc = ((dx_tmp / dz_tmp) - after_points[o].x()) *
    ...
    stdev = residual / inlcnt;

    // remove outliers, Talworth method
    for (int o = 0; o < vertCnt; o++) {
      if ((resid[o] > sigma * sigma * stdev) && weights[o]) {
        weights[o] = false;
      }
    }
    ...
    if (ret == failedDetZero) {
      ...
      if ((inlcnt == oldinlcnt) && (flag)) return true;
    }
    // else     if(ret == succeededBigChange)
    // do next iteration
