...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  if (prog == 0) {
    ...
    Test::Math::Vector<float, 2> mesh_grid[corners[*reference].rows];
    ...
    /*
    for(int j=0; j<64; j++) std::cout << *(reference_corner+j) << std::endl;
    std::cout << std::endl;
    for(int j=0; j<64; j++) std::cout << *(mesh_grid+j) << std::endl;
    */
    ...
    for (int i = 0; i < count; ++i) {
      ...
      // for(int j=0; j<64; j++) std::cout << *(sample_corner+j) << std::endl;

      grid.regPersp(sample_corner, reference_corner, corners[i].rows);
      /*
      for(int j=0; j<9; j++) std::cout << *(grid.getCoeffs()+j)  << "  ";
      std::cout << std::endl;
      */
      /*
      if (i==2)
      {
              *(grid.getCoeffs()+0) = 0.97368;
              *(grid.getCoeffs()+1) = -0.00213;
              *(grid.getCoeffs()+2) = 0.04375;
              *(grid.getCoeffs()+3) = -0.00021;
              *(grid.getCoeffs()+4) = 0.93038;
              *(grid.getCoeffs()+5) = 0.00221;
              *(grid.getCoeffs()+6) = 0.06915;
              *(grid.getCoeffs()+7) = 0.00064;
      }
      */
      ...
      for (int j1 = 0; j1 < 3; j1++)
        for (int j2 = 0; j2 < 3; j2++)
          H.at<double>(j1, j2) = *(grid.getCoeffs() + 3 * j1 + j2);

      H.at<double>(2, 2) = 1;
      double vals1[] = {2.0 / img[i].width,
                        0.0,
                        -1.0,
                        0.0,
                        2.0 / img[i].height,
      ...
      double vals2[] = {img[i].width / 2.0,
                        ...
                        0.0,
                        1.0};
      cv::Mat Transf(3, 3, CV_64F, vals1);
      cv::Mat TransfI(3, 3, CV_64F, vals2);
      cv::Mat H1 = H * Transf;
      H = TransfI * H1;
      H /= H.at<double>(2, 2);
      ...
      /*
      std::cout << (*(grid.getCoeffs()+0)*1 + *(grid.getCoeffs()+1)*1 +
      *(grid.getCoeffs()+2))/
              (*(grid.getCoeffs()+6)*1 + *(grid.getCoeffs()+7)*1 + 1) <<
      std::endl; std::cout << (*(grid.getCoeffs()+3)*1 + *(grid.getCoeffs()+4)*1
      + *(grid.getCoeffs()+5))/
              (*(grid.getCoeffs()+6)*1 + *(grid.getCoeffs()+7)*1 + 1) <<
      std::endl;
      */

      cv::Mat H2 = findHomography(corners[i], corners[*reference], CV_RANSAC);
      std::cout << H2.inv() << std::endl;

      /*
      std::cout << (H.at<double>(0,0)*1 + H.at<double>(0,1)*1 +
      H.at<double>(0,2))/ (H.at<double>(2,0)*1 + H.at<double>(2,1)*1 +
      H.at<double>(2,2)) << std::endl; std::cout << (H.at<double>(1,0)*1 +
      H.at<double>(1,1)*1 + H.at<double>(1,2))/ (H.at<double>(2,0)*1 +
      H.at<double>(2,1)*1 + H.at<double>(2,2)) << std::endl;
      */
      /*
      // Select the inlier features
      Test::Math::Vector<float,2> *temp_corner =
      (Test::Math::Vector<float,2>*)corners[i].data; Test::Math::Vector<float,2>
      inlier_corner[countNonZero(status[i])]; Test::Math::Vector<float,2>
      reference_corner[countNonZero(status[i])]; for(int j=0, j1=0;
      j<corners[i].rows; j++) if (status[i].at<unsigned char>(j,0)==1)
              {
                      inlier_corner[j1]=*(temp_corner+j);
                      reference_corner[j1]=*(temp_reference_corner+j);
                      j1++;
              }

      std::cout<<std::endl;
      for(int j=0; j<corners[i].rows; j++) std::cout << inlier_corner[j] << " "
      << reference_corner[j] << std::endl;
      */

      // std::cout << corners[i] << std::endl;
      // std::cout << grid.regPersp(inlier_corner, WARP_GRID_SIZE_X,
      // WARP_GRID_SIZE_Y) << std::endl; float *coeffr = new float [9];
      // coeffr=grid.getCoeffs();
      // for(int j=0; j<9; j++) std::cout << coeffr[j] << std::endl;

      // for(int j=0; j<status[i].size(); j++) if
      // std::cout << corners[i].cols << "  " << corners[i].rows << std::endl;
      std::cout << corners[i] << std::endl;
      // std::cout << corners[i].at<float>(0,0) << "  " <<
      // corners[i].at<float>(0,1) << std::endl;
      ...
      cv::warpAffine(mO[i].clone(), mO[i], cv::Mat(2, 3, CV_64F, coeff[i]),
      m[i].size());
      */
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    }
