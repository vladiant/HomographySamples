...
*/
/*
const float uv_adj_from_y255[256] = {
#include "uv_adj_from_y255_lin1.8_012.txt"
};
*/
...
int32_t test_rhdr_create(void** context) {
  *context = (rhdr_context_t*)rhdr_malloc(sizeof(rhdr_context_t));
...
/*** test { */
  ...
  rhdr_memset(ctx, 0, sizeof(rhdr_context_t));

  ctx->info.rhdr_version = RHDR_VERSION;
  ctx->din = din;
...
/*** test { */
#ifdef DEBUG_LEVEL
      int ref_pixel = *(pimgs[din->ref_image]);
      if (is_debug_pixel(col, row)) {
        blank();
      }
#endif
...
/*** test { */
  ...
  /* write_bmp(din->out_rgb, din->img_sx, din->img_sy, "rhdr_out", ".bmp"); */
