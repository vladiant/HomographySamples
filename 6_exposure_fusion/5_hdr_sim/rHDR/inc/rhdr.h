...
/* @def rhdr_malloc, rhdr_free
alloc and free functions from OS in current framework
*/
#define rhdr_malloc(size) malloc(size)
#define rhdr_free(size) free(size)

/* @def rhdr_memset
function for filling memory with a number from string.h of this particular
framework
*/
#define rhdr_memset(dst, val, bytes) memset(dst, val, bytes)
