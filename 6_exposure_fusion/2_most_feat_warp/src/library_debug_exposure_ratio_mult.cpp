...
int hdr_proc_files(const char** input_files, const char** output_files,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    img[i] = jpeg[i].getImg();
    // gamma_reverse(img[i].buffer, img[i].buffer, img[i].width, img[i].height,
...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  if (*reference < 0 || *reference >= count) {
    ...
    /*
    // Lets choose reference to be the image with least/most exposure
    float ext_exp = exposure[0];
    *reference = 0;
    for(int i = 1; i < count; ++i)
            if (exposure[i] > ext_exp)
                    {
                            *reference = i;
                            ext_exp = exposure[i];
                    }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // gray[i].getWidth()*gray[i].getHeight()); std::cout << exposure[i] <<
    // std::endl;
    /*
    ...
    */
    ...
    /*
    float exp_ratio=exposure[*reference]/exposure[i];  //min exposure
  ...
  for (int i = 0; i < count; ++i) {
    ...
    /*
    char buf1[] = "color0.png";
    buf1[5] = '0'+i;
    cv::Mat mtemp = cv::Mat(img[i].height, img[i].width, CV_8UC3,img[i].buffer);
    gamma_reverse(img[i].buffer, mtemp.data, img[i].width, img[i].height,
    img[i].width, img[i].width); cv::imwrite(buf1,mtemp);
    */
  }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (exposure[i] > exposure[*reference]) {
      ...
      cv::calcOpticalFlowPyrLK(
          m[i], mtemp,                // m[*reference],	// src image,  dst image
          corners[i], refcorners[i],  // src points, dst points
      ...
      cv::calcOpticalFlowPyrLK(
          m[*reference], mtemp,             // m[i],		// src image,  dst image
