...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  fd.setContainer(&seg);
  fd.detect(img[*reference].buffer, img[*reference].width,
            img[*reference].height, img[*reference].width, THRESHOLD);
  fd.removeClose(MAX_DIST);

  for (int j = 0; j < seg.segmentGetCount(); ++j) {
    if (seg[j].status) {
      corners[*reference].push_back(cv::Point2f(seg[j].x, seg[j].y));
    }
  }

  std::cout << "Reference features count:  " << corners[*reference].size()
            << std::endl;
  // corners[*reference].create(points.size(), 2, CV_32FC1);
  /*
  for(int j = 0; j < points.size(); ++j)
  {
          //std::cout << points[j] << std::endl;
          corners[*reference].at<float>(j,0) = points[j].x;
          corners[*reference].at<float>(j,1) = points[j].y;
  }
  */
  ...
  for (int i = 0; i < count; ++i) {
    ...
    THRESHOLD); fd.removeClose(30);

    for(int j = 0; j < seg.segmentGetCount(); ++j)
    {
    //std::cout << seg[j].x << "  " << seg[j].y;
    if(seg[j].status)
            {
                    //std::cout << "  *";
                    //std::cout << j << "  " << seg[j].x << "  " << seg[j].y <<
    std::endl;
            }
    //std::cout << std::endl;
    }
    ...
    cTJpeg temp1;
    temp1.create(img[i].width, img[i].height, img[i].channels);
    for(int j=0; j<img[i].width*img[i].height*img[i].channels;j++)
    *(temp.getData()+j) = 0;// *(img[i].buffer+j); for(int j=0;
    j<img[i].width*img[i].height*img[i].channels;j++) *(temp1.getData()+j) =
    0;//*(img[i].buffer+j);

    fd.draw<3>(temp.getData(), img[i].width, img[i].height, 0xFFFFFF);
    fd.removeClose(30);
    fd.draw<3>(temp1.getData(), img[i].width, img[i].height, 0xFFFFF);

    temp.fileSave("test.jpg", 100, JCS_RGB);
    temp.fileSave("test1.jpg", 100, JCS_RGB);
    */
  ...
  if (prog == 0) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (i != (*reference)) {
        ...
        for (int p = 0; p < corners[*reference].size(); ++p) {
          ...
          if ((dx <= minDistanceX) && (dy <= minDistanceY)) {
            ...
            size++;
            // std::cout << dx << "  " << dy << "  " << minDistanceX << "  " <<
            // minDistanceY << std::endl;
          }
        }
      ...
      for (int p = 0; p < sampl_corn.size(); ++p) {
        cv::circle(mO[i],
                   // corners[i][p], 5, cv::Scalar(0, 0, 0), 2);
                   sampl_corn[p], 5, cv::Scalar(0, 0, 0), 2);
        cv::circle(mO[i],
                   // corners[i][p], 2, cv::Scalar(255, 0, 0), 3);
                   sampl_corn[p], 2, cv::Scalar(255, 0, 0), 3);
      }
      ...
      if (i != *reference) {
        ...
        for (int p = 0; p < sampl_corn.size(); ++p) {
          cv::line(mO[i],
                   ...
                   sampl_corn[p],
                   // corners[*reference][p],
                   ref_corn[p], cv::Scalar(255, 0, 0), 2);
        }
      }
    }
    t[1] = getuTime();
