...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    */
    ...
    gray[i].create(320, 240, 1);  // QVGA
    cTJpeg tempgray;
    tempgray.create(img[i].width, img[i].height, 1);
    // rgb24_to_gray_gamma_reverse(img[i].buffer, tempgray.getData(),
    // img[i].width, img[i].height, img[i].width, img[i].width);
    rgb24_to_gray(img[i].buffer, tempgray.getData(), img[i].width,
                  img[i].height, img[i].width, img[i].width);
    cv::Mat mtemp = cv::Mat(tempgray.getHeight(), tempgray.getWidth(), CV_8UC1,
                            tempgray.getData());
    cv::Mat mvga = cv::Mat(480, 640, CV_8UC1);

    cv::blur(mtemp, mtemp, cv::Size(7, 7));
    // cv::GaussianBlur(mtemp, mtemp, cv::Size(0,0), 7);

    /*
    cv::Mat mtempL = cv::Mat(tempgray.getHeight(), tempgray.getWidth(),
    CV_32FC1); cv::Laplacian(mtemp, mtempL, mtempL.depth()); double maxL, minL;
    cv::Point dummy;
    cv::minMaxLoc(mtempL, &minL, &maxL);
    for(int j1=0; j1<mtempL.rows;j1++)
            for(int j2=0; j2<mtempL.cols;j2++)
            {
                    float temp = mtempL.at<float>(j1,j2);
                    mtemp.at<uchar>(j1,j2)=uchar(255*(temp-minL)/(maxL-minL)-128);
            }
    */
    cv::resize(mtemp, mvga, mvga.size(), 0, 0, cv::INTER_NEAREST);
    cv::Mat mqvga = cv::Mat(240, 320, CV_8UC1, gray[i].getData());
  ...
  for (int i = 0; i < count; ++i) {
    ...
    */
    ...
    fd.removeClose(MAX_DIST * gray[i].getWidth() / SEG_X_COUNT);
    ...
    */
    for (int j = 0; j < seg.segmentGetCount(); ++j) {
      if (seg[j].status) {
        ...
        // corners[i].push_back(cv::Point2f(seg[j].x*gray[i].getWidth()/float(tempgray.getWidth()),
        // seg[j].y*gray[i].getHeight()/float(tempgray.getHeight())));
      }
    }

    // Features found, apply gamma
    gamma_reverse_gray(gray[i].getData(), gray[i].getData(), gray[i].getWidth(),
                       gray[i].getHeight(), gray[i].getWidth(),
                       gray[i].getWidth());
    /*
