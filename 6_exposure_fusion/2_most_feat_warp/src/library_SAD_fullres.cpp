...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // rgb24_to_gray_gamma_reverse(img[i].buffer, gray[i].getData(),
    // img[i].width, img[i].height, img[i].width, img[i].width);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    gray[i].getData()); cv::Mat mvga = cv::Mat(480, 640, CV_8UC1);
    ...
    */
    ...
    for(int j = 0; j < feat1[i].segmentGetCount(); ++j)
    {
            if(feat1[i][j].status)
                    {
                            feat1[i][j].x *=
    gray[i].getWidth()/float(tempgray.getWidth()); feat1[i][j].y *=
    gray[i].getHeight()/float(tempgray.getHeight());
                    }
    }
    */
