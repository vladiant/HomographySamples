...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // rgb24_to_gray(img[i].buffer, gray[i].getData(), img[i].width,
    // img[i].height, img[i].width, img[i].width);
  }
  t[1] = getuTime();
  ...
  ///////////////////////////////////
  std::vector<std::vector<cv::Point2f>> corners(count);
  std::vector<std::vector<cv::Point2f>> refcorners(count);
  //////////////////////////////////
  t[0] = getuTime();

  cTFeatureSegmentContainer<float> feat1[count];
  for (int i = 0; i < count; ++i)
    feat1[i].segmentSetCount(SEG_X_COUNT, SEG_Y_COUNT);

  for (int i = 0; i < count; ++i) {
    // calculate features of reference and overexposed images
    if ((i != *reference) && (exposure[i] < exposure[*reference])) continue;
    ...
    fd.setContainer(&feat1[i]);
    /*
    fd.detect(gray[i].getData(), gray[i].getWidth(), gray[i].getHeight(),
    gray[i].getWidth(), THRESHOLD);
    fd.removeCloseF(MAX_DIST*gray[i].getWidth()/SEG_X_COUNT);
    */
    ...
    for (int j = 0; j < feat1[i].segmentGetCount(); ++j) {
      if (feat1[i][j].status) {
        feat1[i][j].x *= gray[i].getWidth() / float(tempgray.getWidth());
        feat1[i][j].y *= gray[i].getHeight() / float(tempgray.getHeight());
      }
    }

    // store reference corners
    if (i == *reference)
      for (int j = 0; j < feat1[*reference].segmentGetCount(); ++j) {
        if (feat1[*reference][j].status) {
          corners[i].push_back(
              cv::Point2f(feat1[*reference][j].x, feat1[*reference][j].y));
          refcorners[i].push_back(
              cv::Point2f(feat1[*reference][j].x, feat1[*reference][j].y));
        }
      }
  }
  ...
  t[0] = getuTime();
  for (int i = 0; i < count; ++i) {
    float distanceSADx = 128;  // MAX_MOTION_VECTOR*gray[i].getWidth();
    float distanceSADy = 128;  // MAX_MOTION_VECTOR*gray[i].getHeight();
    ...
    if (exposure[i] > exposure[*reference]) {
      ...
      cv::calcOpticalFlowPyrLK(
                      m[i], mtemp,				// src image,
      dst image corners[i], refcorners[i],		// src points, dst
      points status[i], err[i], cv::Size(21, 21), 3);
      */

      cTFeatureSegmentContainer<float> feat2;
      feat2.segmentSetCount(SEG_X_COUNT, SEG_Y_COUNT);

      bruteForceSad(gray[i].getData(), graytemp.getData(), graytemp.getWidth(),
                    graytemp.getHeight(), &feat1[i], &feat2, 16, 16,
                    distanceSADx, distanceSADy);

      for (int j = 0; j < feat2.segmentGetCount(); ++j) {
        if (feat1[i][j].status) {
          corners[i].push_back(cv::Point2f(feat1[i][j].x, feat1[i][j].y));
          refcorners[i].push_back(cv::Point2f(feat2[j].x, feat2[j].y));
        }
      }
      ...
      std::cout << "\n=================Reference===============" << std::endl;
      std::cout << refcorners[i] << std::endl;
      ...
      /*
      cv::calcOpticalFlowPyrLK(
              m[*reference], mtemp,				// src image,
      dst image corners[*reference], corners[i],		// src points,
      dst points status[i], err[i], cv::Size(21, 21), 3);
      */

      cTFeatureSegmentContainer<float> feat2;
      feat2.segmentSetCount(SEG_X_COUNT, SEG_Y_COUNT);

      bruteForceSad(gray[*reference].getData(), graytemp.getData(),
                    graytemp.getWidth(), graytemp.getHeight(),
                    &feat1[*reference], &feat2, 16, 16, distanceSADx,
                    distanceSADy);

      for (int j = 0; j < feat2.segmentGetCount(); ++j) {
        if (feat1[*reference][j].status) {
          corners[i].push_back(cv::Point2f(feat2[j].x, feat2[j].y));
          refcorners[i].push_back(
              cv::Point2f(feat1[*reference][j].x, feat1[*reference][j].y));
        }
      }
