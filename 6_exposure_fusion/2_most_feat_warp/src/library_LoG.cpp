...
int hdr_proc(cTImg* img, int count, int* reference, int prog) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    */
    ...
    cv::Mat mtempL =
        cv::Mat(tempgray.getHeight(), tempgray.getWidth(), CV_32FC1);
    cv::Laplacian(mtemp, mtempL, mtempL.depth());
    double maxL, minL;
    cv::Point dummy;
    ...
    for (int j1 = 0; j1 < mtempL.rows; j1++)
      for (int j2 = 0; j2 < mtempL.cols; j2++) {
        ...
        mtemp.at<uchar>(j1, j2) =
            uchar(255 * (temp - minL) / (maxL - minL) - 128);
      }
